 ...
 *
 * @author 2024 Thomas CORDONNIER
 */
public class PropagateFilter implements IEditorFilter {
    private final Set<Integer> entriesList = new HashSet<Integer>();
    private FilterBarPropagate controlComponent;
    private PrepareTMXEntry pte;

    public PropagateFilter(List<Integer> entries, PrepareTMXEntry pte) {
        this.pte = pte;
        entriesList.addAll(entries);
        controlComponent = new FilterBarPropagate();
        ...
        controlComponent.btnApplyHere.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                apply();
            }
        });        
    ...
    private void apply() {
        ...
        //Core.getProject().setTranslation(ec.getCurrentEntry(), pte, false, null);
        ec.replaceEditText(pte.translation); ec.commitAndDeactivate();
        // skip to next
        skip();
    }    
    ...
    @Override
    public boolean displayOtherSegments() {
        return true;
    }
